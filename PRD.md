...
|--------|----------|--------|------|----------|
| F001 | Notion → JIRA 同步 | P0 | 通过Sync2JIRA按钮触发需求同步到JIRA | 点击按钮后5分钟内在JIRA中创建对应Issue |
| F002 | 字段映射转换 | P0 | 将Notion字段准确映射到JIRA字段 | 所有核心字段正确映射，无数据丢失 |
| F003 | 重复检测机制 | P0 | 避免重复创建JIRA Issue | 同一Notion页面不会创建多个JIRA Issue |
| F004 | Notion状态回写 | P0 | 同步完成后更新Notion页面状态和链接 | 状态改为"已输入"，JIRA链接写入"JIRA CARD"字段 |
| F005 | JIRA → Notion 反向同步 | P0 | JIRA状态变更同步回Notion | 定期检测JIRA变更，更新对应Notion页面 |
| F006 | 增量同步机制 | P0 | 只同步有变更的JIRA Issue | 基于时间戳的增量检测，避免重复处理 |

#### 扩展功能（后续版本）
| 功能ID | 功能名称 | 优先级 | 描述 |
|--------|----------|--------|------|
| F007 | 同步状态监控面板 | P1 | Web界面监控同步状态 |
| F008 | 批量同步功能 | P2 | 支持批量选择多个需求同步 |
| F009 | 冲突解决机制 | P2 | 处理双向同步的数据冲突 |
...
1. **用户操作**: 用户在Notion页面点击"Sync2JIRA"按钮
2. **触发同步**: 系统接收到webhook事件，开始同步处理
3. **字段映射**: 将Notion字段按照映射规则转换为JIRA字段
4. **创建Issue**: 在JIRA中创建对应的Issue
5. **状态回写**: 
   - 将Notion页面的Status字段更新为"已输入"
   - 将JIRA Issue链接写入"JIRA CARD"字段
6. **完成确认**: 用户可以看到状态变更和JIRA链接
